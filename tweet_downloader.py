...
parser.add_argument("TwitterID", type=str, help="Account ID like @XXXXX on Twitter")
parser.add_argument("Max", type=int, help="Max number of Download files")
parser.add_argument("--save", type=str, default="./", help="save file name")
...
if __name__ == '__main__':
    ...
    getter = TweetsGetter.byUser(args.TwitterID)
    ...
    for tweet in getter.collect(total = args.Max):
    ...
    # データフレームに取得したデータを格納
    df = pd.DataFrame(columns=['text', 'like', 'retweet', 'id', 'user', 'created_at'])
    df_new = df.assign(text=list_text, like=list_favorite, retweet=list_retweet, id=list_id, user=list_user_screenname, created_at=list_created_at)

    # save
    save_dir = './csv'
    if not os.path.isdir(save_dir):
        os.mkdir(save_dir)
    df_new.to_csv("./{}.csv".format(os.path.join(save_dir, args.TwitterID)), encoding="shift_jis", errors="ignore")
    # print(df_new)